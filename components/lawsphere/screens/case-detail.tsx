  ...
  return (
    <div className="flex h-full flex-col bg-[var(--color-neutral-50)]">
      ...
      <header className="flex items-center gap-2 border-b border-gray-100 bg-white px-3 py-2">
        ...
        <p className="flex-1 truncate text-sm font-bold text-[var(--color-neutral-950)]">Chi tiết yêu cầu</p>
      </header>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="bg-white px-4 pb-4 pt-4">
          <div className="flex items-start justify-between gap-2">
            <h1 className="text-lg font-bold leading-snug text-[var(--color-neutral-950)] text-balance">{req.title}</h1>
          ...
          <div className="mt-1 flex items-center gap-2">
            ...
            <span className="text-xs text-[var(--color-neutral-500)]">{req.date}</span>
          </div>
          {detail && <p className="mt-3 text-[13px] leading-relaxed text-[var(--color-neutral-500)]">{detail.description}</p>}
        </div>

        {/* Assigned lawyer */}
        <div className="mt-2 bg-white px-4 py-4">
          <h2 className="text-sm font-bold text-[var(--color-neutral-950)]">Luật sư phụ trách</h2>
          {detail?.lawyer ? (
            <div className="mt-3 flex items-center gap-3 rounded-2xl border border-gray-100 bg-white p-3">
              ...
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-bold text-[var(--color-neutral-950)]">{detail.lawyer.name}</p>
                <p className="text-xs text-[var(--color-neutral-500)]">{detail.lawyer.specialty}</p>
              </div>
              <button
                type="button"
                aria-label="Nhắn tin"
            ...
            <div className="mt-3 flex flex-col items-center gap-2 rounded-2xl border border-dashed border-gray-200 bg-[var(--color-neutral-50)] px-4 py-6 text-center">
              <span className="grid h-10 w-10 place-items-center rounded-full bg-white text-[var(--color-neutral-500)]">
                <UserPlus className="h-5 w-5" />
              </span>
              <p className="text-xs text-[var(--color-neutral-500)]">Đang chờ hệ thống ghép nối luật sư phù hợp...</p>
            </div>
          )}
        </div>
        ...
        {/* Timeline */}
        <div className="mt-2 bg-white px-4 py-4">
          <h2 className="text-sm font-bold text-[var(--color-neutral-950)]">Tiến trình xử lý</h2>
          <div className="mt-4">
              ...
              return (
                <div key={ev.title} className="flex gap-3">
                  <div className="flex flex-col items-center">
                    <div
                      className={`grid h-7 w-7 shrink-0 place-items-center rounded-full text-[10px] font-bold ${
                        ev.done ? "bg-[var(--color-primary)] text-white" : "border-2 border-gray-200 bg-white text-gray-300"
                      }`}
                    >
                  ...
                  <div className={`pb-5 ${last ? "" : ""}`}>
                    <p className={`text-[13px] font-semibold ${ev.done ? "text-[var(--color-neutral-950)]" : "text-[var(--color-neutral-500)]"}`}>
                      {ev.title}
                    </p>
        ...
        {detail && detail.documents.length > 0 && (
          <div className="mt-2 bg-white px-4 py-4">
            <h2 className="text-sm font-bold text-[var(--color-neutral-950)]">Tài liệu đính kèm</h2>
            <div className="mt-3 space-y-2">
              {detail.documents.map((d) => (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-[13px] font-semibold text-[var(--color-neutral-950)]">{d.name}</p>
                    <p className="text-xs text-[var(--color-neutral-500)]">{d.size}</p>
                  </div>
                  <button
                    type="button"
