      ...
      <div className="shrink-0 border-b border-gray-100 bg-white px-4 py-3">
        <div className="flex items-center justify-between gap-2">
          ...
          <button
            ...
            className="grid h-8 w-8 place-items-center rounded-lg bg-[#E6F0F9] text-[#2854A8] transition-colors hover:bg-[#D4E4F0]"
        ...
        {/* Search */}
        <div className="mt-3 flex items-center gap-2 rounded-xl bg-[#F5F5F5] px-3 py-2">
          <Search className="h-4 w-4 text-[#5E5E5E]" />
          <input
            type="text"
        ...
        {view === "browse" ? (
          <div className="space-y-3">
            {filteredLawyers.length > 0 ? (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <h3 className="font-semibold text-[#1A1A1A]">{lawyer.name}</h3>
                      {lawyer.online && (
                        <div className="h-2 w-2 rounded-full bg-green-500" title="Đang hoạt động" />
                      )}
                    </div>
                    <p className="text-xs text-[#5E5E5E]">{lawyer.specialty}</p>
              ...
              <div className="flex h-32 items-center justify-center text-center">
                <p className="text-sm text-[#5E5E5E]">Không tìm thấy luật sư phù hợp</p>
              </div>
            )}
          </div>
        ) : (
          <div className="space-y-3">
            {threads.length > 0 ? (
              threads.map((t) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-[#5E5E5E]">{t.specialty}</p>
                    <p className="mt-1 truncate text-xs text-[#666666]">{t.preview}</p>
                  </div>
              ...
              <div className="flex h-32 items-center justify-center text-center">
                <p className="text-sm text-[#5E5E5E]">Chưa có cuộc trò chuyện nào</p>
              </div>
            )}
          </div>
        )}
    ...
    <div className="flex h-full flex-col overflow-hidden bg-[#F5F5F5]">
      ...
      <div className="shrink-0 border-b border-gray-100 bg-white px-4 py-3">
        <button
          type="button"
          onClick={onBack}
          className="mb-3 text-sm font-semibold text-[#2854A8] transition-opacity hover:opacity-70"
        >
          ← Quay lại
        </button>
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <h1 className="text-base font-bold text-[#1A1A1A]">{lawyer.name}</h1>
            <p className="text-xs text-[#5E5E5E]">{lawyer.specialty}</p>
            <div className="mt-2 flex items-center gap-2 text-[11px] text-[#666666]">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4">
        <div className="space-y-4">
          {/* Bio */}
          <div>
            <p className="text-xs font-semibold text-[#5E5E5E] uppercase">Giới thiệu</p>
            <p className="mt-2 text-sm text-[#333333] leading-relaxed">{lawyer.bio}</p>
          </div>

          {/* Experience */}
          <div>
            <p className="text-xs font-semibold text-[#5E5E5E] uppercase">Kinh nghiệm</p>
            <p className="mt-2 text-sm font-semibold text-[#2854A8]">{lawyer.experience} năm</p>
          </div>

          {/* Stats */}
          <div className="grid grid-cols-3 gap-2 rounded-xl bg-white p-3">
            <div className="text-center">
              <p className="text-xl font-bold text-[#2854A8]">{lawyer.cases}</p>
              <p className="mt-0.5 text-[10px] text-[#666666]">Hồ sơ</p>
            </div>
            <div className="border-l border-r border-gray-100 text-center">
              <p className="text-xl font-bold text-[#2854A8]">{lawyer.rating}</p>
              <p className="mt-0.5 text-[10px] text-[#666666]">Đánh giá</p>
            </div>
            <div className="text-center">
              <p className="text-sm font-bold text-[#2854A8]">{lawyer.responseTime}</p>
              <p className="mt-0.5 text-[10px] text-[#666666]">Phản hồi</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="shrink-0 border-t border-gray-100 bg-white px-4 py-3 space-y-2">
        ...
        <button
          type="button"
          className="w-full rounded-xl border border-[#2854A8] py-3 text-sm font-semibold text-[#2854A8] transition-colors hover:bg-[#E6F0F9]"
        >
          Nhắn tin
        </button>
      </div>
